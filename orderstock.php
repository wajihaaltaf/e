...
<?php
$id = $_GET['id'];
	$select = "SELECT * FROM `stocklist`,itemcategory,stockremaining WHERE itemcategory.icategory_id=stocklist.icategory_id and stockremaining.item_id=stocklist.item_id and`stocklist`.`item_id`='$id'";
	//echo $select;
$qry=mysql_query($select);
		$rec = mysql_fetch_array($qry);
		$name = "$rec[item_name]";
		$catname = "$rec[icategory_name]";
		$company = "$rec[item_company]";
...
window.location = "orderstock.php";
</script>
			<?php
			exit();
			}
		...
		$pid=$_SESSION['id'];
		$itemamount = addslashes("$_POST[amount]");
		$iam=$itemamount*$price;
		mysql_query("INSERT INTO `stockorder` (`order_id`, `item_id`, `order_time`, `order_itemamount`, `order_by`, `order_totalcost`, `order_earlypayment`, `order_remainingpayment`, `order_recieved`, `order_approve`) VALUES (NULL, '$id', '', '	$itemamount', '$pid', '$iam', '', '', '0', '0');")or die(mysql_error()); 
		mysql_query("UPDATE `stockremaining` SET `item_isordered` = '0' WHERE `stockremaining`.`strid` = '$id'")or die(mysql_error()); ?>
        <script>
alert('Updated Successfully');
window.location = "stockorder.php";
</script>
<?php
}}
?>
...
<head>
  ...
  <title>Sales | Order Stock</title><link rel="shortcut icon" href="assets/img/logocalc1.png">
    ...
    <section class="content">
     <div class="box box-default">
            <div class="box-header with-border">
              <h3 class="box-title">Order Stock</h3>
		...
		<div class="col-md-12">
			...
			<div> 
					<div class="form-group">
							  <label class="col-md-5 control-label" for="rental">Product Name:</label>
							  <div class="col-md-3">
					<input type="text" name="emp_full_name" id = "fname" class="form-control input-md" placeholder="please enter first name" pattern="[A-Za-z. ]{1,50}" value= "<?php echo $name; ?>" required readonly/> 
						</div>
						</div>
                ...
                <div class="form-group">
							  ...
							  <div class="col-md-3">
						<input type="text" name="emp_email" id = "email" class="form-control input-md"  placeholder="Email" pattern="[^@]+@[^@]+\.[a-zA-Z]{2,}" title="Incorrect Email" value="<?php echo $catname; ?>"required readonly/>
					</div>
				</div>
                 <div class="form-group">
							  <label class="col-md-5 control-label">From Company:</label>
							  <div class="col-md-3">
						<input type="text" name="emp_nic" id = "nic" class="form-control input-md"  placeholder="NIC" title="Numbers Only" value="<?php echo $company; ?>" required readonly/>
					</div>
				</div>	
				<div class="form-group">
							  <label class="col-md-5 control-label" for="rental">Price:</label>
							  <div class="col-md-3">
						<input type="date" name="emp_dob" id = "bdate" title="click to choose a date" class="form-control input-md" value = <?php echo $price; ?> required readonly/>
					</div>
				</div>
               
                <div class="form-group">
							  <label class="col-md-5 control-label" for="rental">Item amount:</label>
							  <div class="col-md-3">
						<input type="text" name="amount" id = "address" class="form-control input-md" placeholder="Amount" pattern="[0-9]{1,20}"  required/>
					</div>
				</div>
               ...
               <div class="control-group">
				<div class="controls" align="center">
						           				 <button name="update" class="btn btn-success">Send to Admin</button>
												 <a button id="cancel" name="cancel" class="btn btn-danger" href="stockorder.php" >Cancel</button></a>
						           			 </div></div>
											 <br>
											 <br>
											 <br>
