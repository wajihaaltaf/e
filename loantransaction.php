...
<?php
$id=$_GET['id'];
$select = "SELECT emp_id FROM leavereq WHERE leave_id= $id";
	...
	$amount = $_POST['lamount'];
		...
		$iid = $rec['ld'];
	$select = "SELECT DATE_FORMAT('$idate', '%Y') - DATE_FORMAT('$sdate', '%Y') - (DATE_FORMAT('$idate', '00-%m-%d') < DATE_FORMAT('$sdate', '00-%m-%d')) AS age FROM loan where loan.loan_id = $iid limit 1";
	$qry=mysql_query($select);
		$rec = mysql_fetch_assoc($qry);
		$age= $rec['age'];
$years = $age * 12;
$installements = $amount/$years;
echo $installements;
	mysql_query("UPDATE `loan` SET `loan_installment` = '$installements' WHERE `loan`.`loan_id` = $iid")or die(mysql_error());
	mysql_query("UPDATE leavereq SET leave_approve=2 where leave_id = $id")or die(mysql_error()); 	
?>
<script>
		...
		<div class="col-md-12">
			...
			<div>
				<div class="form-group">
							  ...
							  <div class="col-md-3">
					 <input type="text"  class="form-control" id="emp_name" name="lamount" placeholder="Amount"required autofocus>
				</div>
				</div>
                <div class="form-group">
				...
				<div class="controls" align="center">
												 ...
												 <a button id="cancel" name="cancel" class="btn btn-danger" href="loan.php" >Cancel</button></a>
						           			 </div></div>
											 <br>
											 <br>
											 <br>
